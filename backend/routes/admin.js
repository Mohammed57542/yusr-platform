...
router.delete('/users/:id', (req, res) => {
  ...
  try {
    ...
    db.prepare('DELETE FROM referrals WHERE ambassador_id = ? OR referred_user_id = ?').run(userId, userId);
...
router.post('/content-exams', (req, res) => {
  const { grade_id, subject_id, unit_id, title, description, duration_minutes, question_count, exam_type } = req.body;
  ...
  const result = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(Number(grade_id), Number(subject_id), unit_id ? Number(unit_id) : null, title, description || '',
    Number(duration_minutes), Number(question_count), exam_type || 'درس');
...
router.patch('/content-exams/:id', (req, res) => {
  ...
  const { grade_id, subject_id, unit_id, title, description, duration_minutes, question_count, exam_type } = req.body;
  db.prepare(`
    UPDATE exams SET grade_id = ?, subject_id = ?, unit_id = ?, title = ?, description = ?, duration_minutes = ?, question_count = ?, exam_type = ? WHERE id = ?
  `).run(
...
router.get('/content-live-sessions', (req, res) => {
  res.json(db.prepare(`
    ...
    ORDER BY ls.session_date DESC, ls.id DESC
  `).all());
});

router.post('/content-live-sessions', (req, res) => {
  const { grade_id, subject_id, title, teacher_name, session_date, session_time, status, meeting_url, video_url } = req.body;
  if (!grade_id || !subject_id || !title) return res.status(400).json({ error: 'الرجاء إدخال الصف والمادة والعنوان' });
  const result = db.prepare(`
    INSERT INTO live_sessions (grade_id, subject_id, title, teacher_name, session_date, session_time, status, meeting_url, video_url)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(Number(grade_id), Number(subject_id), title, teacher_name || null, session_date || null, session_time || null, status || 'upcoming', meeting_url || null, video_url || null);
...
router.patch('/content-live-sessions/:id', (req, res) => {
  ...
  const { grade_id, subject_id, title, teacher_name, session_date, session_time, status, meeting_url, video_url } = req.body;
  db.prepare(`UPDATE live_sessions SET grade_id = ?, subject_id = ?, title = ?, teacher_name = ?, session_date = ?, session_time = ?, status = ?, meeting_url = ?, video_url = ? WHERE id = ?`)
    .run(grade_id !== undefined ? Number(grade_id) : session.grade_id,
...
router.delete('/content-live-sessions/:id', (req, res) => {
  const result = db.prepare('DELETE FROM live_sessions WHERE id = ?').run(Number(req.params.id));
